...
__global__ void cross_floyed(int n, int p, int *graph, int *corss_row, int *corss_col) {
    ...
    for (int k = 0; k < Max_p_SIZE; ++k) 
        for (int l = 0; l < Max_p_SIZE; ++l) {
            if (is_row) corss_row[(ty + k) * n + (blockIdx.x * MAX_P_SIZE + tx + l)] = k + i < n && l + j < n ? local[k][l] : INF;
            else corss_col[(blockIdx.x * MAX_P_SIZE + ty + k) * MAX_P_SIZE + (tx + l)] = k + i < n && l + j < n ? local[k][l] : INF;
        }
}
...
__global__ void others_floyed(int n, int p, int *graph, int *corss_row, int *corss_col) {
    ...
    for (int k = 0; k < Max_p_SIZE; ++k) {
        #pragma unroll
        for (int l = 0; l < Max_p_SIZE; ++l) 
            shared_row[ty + k][tx + l] = corss_col[(i + k) * MAX_P_SIZE + (tx + l)];
    ...
    for (int k = 0; k < Max_p_SIZE; ++k) {
        #pragma unroll
        for (int l = 0; l < Max_p_SIZE; ++l) {
            shared_col[ty + k][tx + l] = corss_row[(ty + k) * n + (j + l)];
        }
    }
    for (int k = 0; k < Max_p_SIZE; ++k) 
        for (int l = 0; l < Max_p_SIZE; ++l) 
