...
__global__ void others_floyed(int n, int p, int *graph, int *corss_row, int *corss_col) {
    ...
    for (int k = 0; k < Max_p_SIZE; ++k) {
        ...
        for (int l = 0; l < Max_p_SIZE; ++l) {
            ...
            if (shared_row[ty + k][tx + l] != corss_row[(i + k) * MAX_P_SIZE + (tx + l)]) {
                printf("shared_row[%d][%d] = %d, corss_row[%d][%d] = %d\n", ty + k, tx + l, shared_row[ty + k][tx + l], (i + k) * MAX_P_SIZE + (tx + l), p * MAX_P_SIZE + tx + l, corss_row[(i + k) * MAX_P_SIZE + (tx + l)]);
            }
        }
    }
    #pragma unroll
